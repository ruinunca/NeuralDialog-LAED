...
class LightDiVAE(BaseModel):
    def __init__(self, corpus, config):
        super(LightDiVAE, self).__init__(config)
        ...
        self.log_py = nn.Parameter(torch.log(torch.ones(self.config.y_size, self.config.k) / config.k),
                                   requires_grad=True)
        ...
        if mode == GEN:
            ...
            if self.config.use_mutual:
                if self.config.train_prior:
                    reg_kl = self.cat_kl_loss(avg_log_qy,
                                              F.log_softmax(self.log_py, self.log_py.dim()-1),
                                              batch_size,
                                              unit_average=True)
                else:
                    reg_kl = b_pr
            else:
                reg_kl = self.cat_kl_loss(log_qy,
                                          self.log_uniform_y,
                                          batch_size,
                                          unit_average=True)
            ...
            results = Pack(main=main, reg_kl=reg_kl, mi=mi, bpr=b_pr)
